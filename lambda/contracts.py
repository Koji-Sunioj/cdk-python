...
from utils.utils import (serialize_float, serialize_int, epoch_to_date, check_response)
...
def handler(event, context):
    ...
    try:
        if resource == "/contracts" and method == "GET":
            ...
            return_object["body"] = json.dumps(
                {"message": "successfully created contract", "contract": contract}, default=serialize_float)
            ...
            return_object["body"] = json.dumps(
                {"message": message, "contract": response["Attributes"]}, default=serialize_float)
            ...
            start_time, end_time = int(event["pathParameters"]["start_time"]), int(
                event["pathParameters"]["end_time"])
            shifts = get_shifts(contract_id, start_time, end_time)
            response = delete_shifts(contract_id, shifts)
            check_response(response, "could not delete shifts")
            message = "successfully deleted %s shifts starting %s and ending %s under contract %s" % (
                len(shifts), epoch_to_date(start_time), epoch_to_date(end_time), contract_id)
            return_object["body"] = json.dumps({"message": message})
