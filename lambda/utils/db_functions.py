...
def get_shifts(contract_id,start_time,end_time):
    shifts = shifts_table.query(
    KeyConditionExpression="contract_id=:contract_id AND start_time>=:start_time",
    FilterExpression="end_time<=:end_time",
    ProjectionExpression="start_time,end_time",
    ExpressionAttributeValues={":contract_id":contract_id,":start_time":start_time,":end_time":end_time}
    )
    return shifts["Items"]
...
def delete_shifts(contract_id,shifts):
    try:
        with shifts_table.batch_writer() as batch:
            for shift in shifts:
                shifts_table.delete_item(Key={"contract_id":contract_id,"start_time": int(shift["start_time"])})
        response_code = 200
    except:
        response_code = 400
    response = {"ResponseMetadata":{"HTTPStatusCode":response_code}}       
